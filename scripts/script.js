  ...
  ;(function($, undefined) {
    ...
    $.ajax({
        type: "GET",
        url: "CSV/resultat.csv",
        dataType: "text",
    }).done(function(data) {
        const chartData = processCSVData(data);
    ...
    function processCSVData(data) {
        ...
        lines.forEach(line => {
            const cols = line.split(","); // Adaptez le séparateur si nécessaire
            if (cols.length === 2) {
                const title = cols[0]; // Nom de la catégorie
                const value = parseFloat(cols[1]); // Valeur
                if (!isNaN(value)) {
                    chartData.push({
                        title: title,
                        value: value,
    ...
    function drawDoughnutChart(data) {
        $('#doughnutChartContainer').drawDoughnutChart(data, {
            segmentShowStroke: true,
            segmentStrokeColor: "#0C1013",
            segmentStrokeWidth: 1,
            baseColor: "rgba(0,0,0,0.5)",
